...
def handle_part(conf, msg, part, tags):
  ...
  if part.get_content_type() in content_processors:
    ...
    newcontent = f(part.get_payload(decode=True), noOCR=noOcr, language=language, despeckle=despeckle, noPDFText=nopdftext)
...
def main(argv):
  ...
  for uid in map(int, uids[0].split()):
    ...
    if(recode!='OK'):
      ...
      continue
    if(data is None or len(data)==0 or data[0] is None or len(data[0])==0):
